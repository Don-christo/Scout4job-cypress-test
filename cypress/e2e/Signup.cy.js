describe('Sign up a user', () => {
  ...
  it("sign up", () => {
    ...
    cy.url().should('include', 'scout4job')
    .and('eq', 'https://scout4job.com/')
    cy.title().should('include', 'the #1 job site')
    .and('equal', 'Scout4Job | Jobs and Recruitment on scout4job.com, the #1 job site')
    ...
    cy.get("a[class='text-primary ']").should('exist')
    .click()

    // clicks on job seeker button
    cy.get("body > div:nth-child(2) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > form:nth-child(1) > div:nth-child(1) > div:nth-child(4) > ul:nth-child(1) > button:nth-child(1)")
    .should('exist')
    ...
    cy.get('select[name="phoneCountry"]').should('exist')
    .and('not.be.visible')
    .select('NG', { force: true }); // phone country
    cy.get('input[name="phone"]').should('exist')
    .type("08155052702"); // phone number
